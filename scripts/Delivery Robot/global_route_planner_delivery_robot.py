...
agent_start = [agent_posx+15, agent_posy + agent_height/2,0]
agent_goal = [car1_posx+agent_width+50,15 + agent_height/2,0]

# kinematic prameters, as given in the document.
wheelRadius = 1
wheelDist = 10
vel_L = [1,0,-1]
vel_R = [1,0,-1]
...
agent_bound_T = [[-10,10,10,-10],[-5,-5,5,5],[1,1,1,1]] 
...
# to get the boundary of the robot
...
#Heuristic function
def heuristic_function(x,y,theta,vl,vr): 
        global park_point
        theta_ = 0
    ...
    while len(open_set)>0:
        ...
        if not (check_visited([round(shortest[0]),round(shortest[1]),round(shortest[2])],visited)): # check if already visited
            ...
            if shortest[2]>180: 
                ...
                angle = shortest[2]
            if round(shortest[0]) <= agent_goal[0]+1 and round(shortest[0]) >= agent_goal[0]-1 and round(shortest[1]) <= agent_goal[1]+1 and \
                round(shortest[1]) >= agent_goal[1]-1 and angle <= agent_goal[2]+5 and angle >= (agent_goal[2]-5): # goal conditions
                return path
            neighbours= get_neighbours(shortest[0],shortest[1],shortest[2]) # get valid neighbours using the kinematic equation
            for neighbour in neighbours:    #calculate cost of each neighbor
                ...
                temp_tcost = temp_gcost+(0.9*heuristic_function(neighbour[0],neighbour[1],neighbour[2],vel_l,vel_r))
